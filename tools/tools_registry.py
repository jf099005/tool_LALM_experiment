"""The single, project-wide definition of `tool_use_LALM`'s toolset.

`available_tool_names()` / `available_tool_schemas()` / `describe_available_tools()`
are the canonical answer to "which tools exist in this project and are in
scope right now" -- every consumer that needs to know that (not just this
module's own synthetic-data generation) reads it from here rather than
keeping its own independent notion:

- `interface/protocol.py` (`audio_to_audio_tool_names()`, `build_system_prompt()`)
  and `interface/executor.py` (`run_tool_call()`) restrict what the live agent
  advertises and is allowed to actually call to this same set.
- `tool_use_training/gen_1st_stage_data/build_dataset.py` and
  `testing_tool_use_benchmark/build_benchmark.py` (imported as
  `from tools import tools_registry as tool_registry`) draw the tool chains
  they synthesize from it.

This keeps the toolset from drifting into two different lists (e.g. a live
agent that advertises a tool the model was never trained to call, because
training data generation used a narrower list than the agent did).

Beyond that shared surface, each `@register`-decorated entry also wraps an
existing tool implementation from this package (or `audio_edit/`) behind one
synthesis call shape, used only by the dataset generators above to invent a
...
placeholder audio reference). Registration also doubles as the per-tool
availability gate: a tool is only in scope if it's both worth training on and
actually usable in the current interpreter -- run this under the project's
`ms-swift` (or similar) conda env to unlock librosa/soundfile-backed tools.
...
from . import TOOL_NAME_TO_CLASS  # noqa: E402
...
def available_tool_schemas() -> list[Dict[str, Any]]:
    """Model-facing function schemas for every tool active in this registry.

    Looks the real `Tool` subclass up in `tools.TOOL_NAME_TO_CLASS` rather
    than `REGISTRY` itself -- this registry's entries only carry a
    description string and a synthetic-parameter-generating closure (see
    `ToolSpec`/`register` above), not the class needed for
    `Tool.to_function_schema()`.
    """
    return [TOOL_NAME_TO_CLASS[name].to_function_schema() for name in available_tool_names()]


def describe_available_tools(tool_call_format: str = "qwen") -> str:
    """Render the tools preamble text for the system prompt.

    Delegates the actual wire convention to `tool_call_formats.py` -- pass
    `tool_call_format="legacy"` (or add a new `ToolCallFormat`) to render a
    different model family's convention without touching this function.
    """
    from tool_call_formats import get_tool_call_format  # noqa: E402 (REPO_ROOT on sys.path above)

    return get_tool_call_format(tool_call_format).render_tools_preamble(available_tool_schemas())
